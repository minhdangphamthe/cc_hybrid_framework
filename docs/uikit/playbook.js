...
(function () {
  ...
  function filterAll(term) {
    ...
    if (nav) {
      nav.querySelectorAll('a').forEach((a) => {
        const hay = (a.getAttribute('data-text') || a.textContent || '').toLowerCase();
        a.style.display = t === '' || hay.includes(t) ? 'block' : 'none';
      });
    }

    // Filter cards
    cards.forEach((c) => {
      const hay = (c.getAttribute('data-text') || c.textContent || '').toLowerCase();
      c.style.display = t === '' || hay.includes(t) ? 'block' : 'none';
    });
  }

  if (q) {
    q.addEventListener('input', () => filterAll(q.value));
    // Run once (if someone prefilled the input)
    filterAll(q.value);
  }
